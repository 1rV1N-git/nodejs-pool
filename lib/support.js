...
function sendtoTelegram(toChat, msgtext){
    ...
    let uri = "https://api.telegram.org/bot" + global.config.bot_key + "/sendmessage?chat_id=" + toChat + "&text=" + utf8.encode(msgtext);
...
function sendtoDiscord(message){
    if (!global.config.general.discordWebhook) {
        return;
    }
