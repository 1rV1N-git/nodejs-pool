...
function full_stop(err) {
    ...
    global.support.sendEmail(global.config.general.adminEmail, "Payment daemon unable to make payment LOK",
	  ...
	  global.support.sendtoTelegram(global.config.admin_telegram, "Payment daemon unable to make payment LOK");
}
        ...
        function (intCallback) {
            ...
            xmrAPIClient.get('order_parameter_query/', function (err, res, body) {
                if (err) {
                    ...
                    console.log("Attempting to pay: " + address + " Amount: " + amtOfBTC + " BTC or " + amount / global.config.general.sigDivisor + " LOK");
                    console.log("Response from XMR.to: " + JSON.stringify(body));
                    if (body.lower_limit >= amtOfBTC) {
                        return intCallback("Not enough LOK to hit the minimum deposit");
                    } else if (body.upper_limit <= amtOfBTC) {
                        return intCallback("Too much LOK to pay out to xmr.to");
                    } else {
...
function Payee(amount, address, paymentID, bitcoin) {
    ...
    this.makePaymentWithID = function () {
        ...
        paymentQueue.push(paymentDetails, function (body) {
            if (body.fee && body.fee > 10) {
                console.log("[*] Successful payment to " + identifier + " of " + global.support.coinToDecimal(amount) + " LOK (fee " + global.support.coinToDecimal(fee) + " - " + global.support.coinToDecimal(body.fee) + " = " + global.support.coinToDecimal(fee - body.fee) + ") with tx_hash " + body.tx_hash.match(hexChars)[0] + " and tx_key " + body.tx_key);
    ...
    this.trackPayment = function () {
        ...
        global.mysql.query("SELECT email FROM users WHERE username = ? AND enable_payout IS true limit 1", [payee.id]).then(function (rows) {
            ...
            global.support.sendEmail(rows[0].email,
                sprintf("Your %(payment_amount)s LOK payment was just performed", emailData),
                sprintf(
                    "Your payment of %(payment_amount)s LOK (with tx fee %(fee)s LOK) to %(address2)s wallet was just performed and total due was decreased by %(amount)s LOK.\n" +
                    (payee.tx_hash && payee.tx_key ?
                        "Your payment tx_hash (tx_id) is %(tx_hash)s and tx_key is %(tx_key)s (can be used to verify payment)\n" +
                        "Here is link to verify that this payment was made: https://lokiblocks.com/prove/%(tx_hash)s/%(address)s/%(tx_key)s\n" +
        ...
        global.mysql.query("SELECT telegram_id FROM users WHERE username = ? AND enable_tlg_payout IS true limit 1", [payee.id]).then(function (rows) {
            ...
            global.support.sendtoTelegram(rows[0].telegram_id, sprintf(
                    "Your payment of %(payment_amount)s LOK (with tx fee %(fee)s LOK) to %(address)s wallet was just performed and total due was decreased by %(amount)s LOK.\n" +
    ...
    global.mysql.query("SELECT * FROM balance WHERE amount >= ?", [global.support.decimalToCoin(global.config.payout.walletMin)]).then(function (rows) {
        ...
        async.eachSeries(rows, function(row, next) {
            ...
            global.mysql.query("SELECT payout_threshold FROM users WHERE username = ?", [payee.id]).then(function (userRow) {
                let threshold = global.support.decimalToCoin(0.1);
                ...
                paymentQueue.unshift(paymentDetails, function (body) {  //jshint ignore:line
                    ...
                    if (body.fee && body.fee > 10) {
                        ...
                        paymentDetails.destinations.forEach(function (payeeItem) {
                            ...
                            console.log("[**] Successful payment to " + payeeItem.address + " for " + global.support.coinToDecimal(payeeObjects[payeeItem.address].amount) + " LOK (fee " + global.support.coinToDecimal(payeeObjects[payeeItem.address].fee) + ")");
                        });
                        console.log("[*] Successful payment to multiple miners of " + global.support.coinToDecimal(totalAmount) + " LOK (fee " + global.support.coinToDecimal(totalFee) + " - " + global.support.coinToDecimal(body.fee) + " = " + global.support.coinToDecimal(totalFee - body.fee) + ") with tx_hash " + body.tx_hash.match(hexChars)[0] + " and tx_key " + body.tx_key);
                        global.mysql.query("INSERT INTO transactions (bitcoin, address, payment_id, xmr_amt, transaction_hash, mixin, fees, payees) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
