...
function full_stop(err) {
    ...
    global.support.sendEmail(global.config.general.adminEmail, "Payment daemon unable to make payment XCA",
    ...
    global.support.sendtoTelegram(global.config.admin_telegram, "Payment daemon unable to make payment XCA");
}
        ...
        function (intCallback) {
            ...
            xmrAPIClient.get('order_parameter_query/', function (err, res, body) {
                if (err) {
                    ...
                    console.log("Attempting to pay: " + address + " Amount: " + amtOfBTC + " BTC or " + amount / global.config.general.sigDivisor + " XCA");
                    console.log("Response from XMR.to: " + JSON.stringify(body));
                    if (body.lower_limit >= amtOfBTC) {
                        return intCallback("Not enough XCA to hit the minimum deposit");
                    } else if (body.upper_limit <= amtOfBTC) {
                        return intCallback("Too much XCA to pay out to xmr.to");
                    } else {
...
function Payee(amount, address, paymentID, bitcoin) {
    ...
    this.makePaymentWithID = function () {
        ...
        paymentQueue.push(paymentDetails, function (body) {
            if (body.fee && body.fee > 0) {
                console.log("[*] Successful payment to " + identifier + " of " + global.support.coinToDecimal(amount) + " XCA (fee " + global.support.coinToDecimal(fee) + " - " + global.support.coinToDecimal(body.fee) + " = " + global.support.coinToDecimal(fee - body.fee) + ") with tx_hash " + body.tx_hash.match(hexChars)[0] + " and tx_key " + body.tx_key);
    ...
    this.trackPayment = function () {
        ...
        global.mysql.query("SELECT email FROM users WHERE username = ? AND enable_payout IS true limit 1", [payee.id]).then(function (rows) {
            ...
            global.support.sendEmail(rows[0].email,
                sprintf("Your %(payment_amount)s XCA payment was just performed", emailData),
                sprintf(
                    "Your payment of %(payment_amount)s XCA (with tx fee %(fee)s XCA) to %(address2)s wallet was just performed and total due was decreased by %(amount)s XCA.\n" +
        ...
        global.mysql.query("SELECT telegram_id FROM users WHERE username = ? AND enable_tlg_payout IS true limit 1", [payee.id]).then(function (rows) {
            if (rows.length === 0) return;
            // toAddress, subject, body
            let emailData = {
                address: payee.id,
            ...
            global.support.sendtoTelegram(rows[0].telegram_id, sprintf(
                    "Your payment of %(payment_amount)s XCA (with tx fee %(fee)s XCA) to %(address)s wallet was just performed and total due was decreased by %(amount)s XCA.\n" +
                    (payee.tx_hash ? "Your payment tx_hash is %(tx_hash)s (can be used to verify payment)\n" : ""),
                    emailData
                ));
        });
    };
    ...
    global.mysql.query("SELECT * FROM balance WHERE amount >= ?", [global.support.decimalToCoin(global.config.payout.walletMin)]).then(function (rows) {
        ...
        async.eachSeries(rows, function(row, next) {
            ...
            global.mysql.query("SELECT payout_threshold FROM users WHERE username = ?", [payee.id]).then(function (userRow) {
                let threshold = global.support.decimalToCoin(500);
                ...
                if (payee.amount >= threshold) {
                    payee.setFeeAmount();
                    if (payee.bitcoin === 0 && payee.paymentID === null && payee.amount !== 0 && payee.amount > 0 && payee.address.length !== 110) {
                    ...
                    } else if (payee.bitcoin === 0 && payee.paymentID === null && payee.amount !== 0 && payee.amount > 0 && payee.address.length === 110 && (payee.amount >= global.support.decimalToCoin(global.config.payout.exchangeMin) || (payee.amount > threshold && custom_threshold))) {
                ...
                paymentQueue.unshift(paymentDetails, function (body) {  //jshint ignore:line
                    ...
                    if (body.fee && body.fee > 0) {
                        ...
                        paymentDetails.destinations.forEach(function (payeeItem) {
                            ...
                            console.log("[**] Successful payment to " + payeeItem.address + " for " + global.support.coinToDecimal(payeeObjects[payeeItem.address].amount) + " XCA (fee " + global.support.coinToDecimal(payeeObjects[payeeItem.address].fee) + ")");
                        });
                        console.log("[*] Successful payment to multiple miners of " + global.support.coinToDecimal(totalAmount) + " XCA (fee " + global.support.coinToDecimal(totalFee) + " - " + global.support.coinToDecimal(body.fee) + " = " + global.support.coinToDecimal(totalFee - body.fee) + ") with tx_hash " + body.tx_hash.match(hexChars)[0] + " and tx_key " + body.tx_key);
                        global.mysql.query("INSERT INTO transactions (bitcoin, address, payment_id, xmr_amt, transaction_hash, mixin, fees, payees) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
