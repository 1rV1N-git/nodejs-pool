...
function Miner(id, login, pass, ipAddress, startingDiff, messageSender, protoVersion, portType, port, agent, algos, algos_perf, algo_min_time) {
    ...
    if (diffSplit.length > 2) {
        this.error = "Too many options in the login field. Please use monero_address[.payment_id][+difficulty_number] format";
        this.valid_miner = false;
        return;
    }

    if (pass_split.length > 2) {
        this.error = "Too many options in the password field. Please use worker_name[:email] format";
        this.valid_miner = false;
        return;
    }

    if (this.payout in bannedAddresses) { // Banned Address
        this.error = "Permanently banned payment address provided: " + bannedAddresses[this.payout];
        this.valid_miner = false;
        return;
    }
    ...
    if (global.coinFuncs.validateAddress(this.address)) {
        ...
        this.error = "Invalid payment address provided: " + this.address + ". Please use 95_char_long_monero_wallet_address[+difficulty_number] format";
        this.valid_miner = false;
        return;
    }
        ...
        setInterval(function () {
            if (donationsShares2 > 0){
                global.coinFuncs.donation.forEach(function (obj) {
                    global.database.storeShare(activeBlockTemplates[""].height, global.protos.Share.encode({
                        shares: 1,
                        ...
                        poolType: 0,
                        poolID: global.config.pool_id,
                        blockDiff: activeBlockTemplates[""].difficulty,
                        bitcoin: 0,
                        blockHeight: activeBlockTemplates[""].height,
                        timestamp: Date.now(),
                        identifier: obj.name + " donations",
                        port: activeBlockTemplates[""].port,
                        share_num: 1
                    }));
                })
            donationsShares2 = 0;
