...
function handleMinerData(socket, method, params, ip, portData, sendReply, sendFinalReply, pushMessage) {
    let miner;
    ...
    switch (method) {
        case 'login':
            ...
            const miner_agent_not_supported_notification = global.coinFuncs.get_miner_agent_not_supported_notification(params.agent);
            if (miner_agent_not_supported_notification) {
                if (!(miner.payout in lastMinerLogTime) || time_now - lastMinerLogTime[miner.payout] > 10*60*1000) {
                    console.error("Sent notification to " + miner.logString + ": " + miner_agent_not_supported_notification);
                    lastMinerLogTime[miner.payout] = time_now;
                }
                sendFinalReply(miner_agent_not_supported_notification);
                return;
            }
...
if (cluster.isMaster) {
    ...
    async.each(global.config.ports, function (portData) {
        ...
        let handleMessage = function (socket, jsonData, pushMessage) {
            ...
            handleMinerData(socket, jsonData.method, jsonData.params, socket.remoteAddress, portData, sendReply, sendFinalReply, pushMessage);
        };
