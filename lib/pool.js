...
const nonceCheck   = new RegExp("^[0-9a-f]{8}$");
...
//Donations
let donationsShares2 = 0;
let reDonationFee = /[%][0-9]{1,3}([\.][0-9]{1,5})?[%]/;
    ...
    function retargetMiner(miner) {
        if (miner.fixed_diff) {
            ...
            if (miner.difficulty * 10 < newDiff) {
                ...
                if (miner.setNewDiff(newDiff)) miner.sendSameCoinJob();
            }
        } else {
...
function updateActiveConfig() {	
    global.mysql.query("SELECT * FROM config WHERE module = 'pool'").then(function (rows) {	
        if (rows.length == 0) {	
            console.error("Can't get pool.config value");	
            return;	
        }	
        rows.forEach(function (row) {	
            if (row.item == "targetTime" && parseInt(row.item_value) != global.config.pool.targetTime) {	
                console.log(threadName + "Changing targetTime time to: " + parseInt(row.item_value))	
                global.config.pool.targetTime = parseInt(row.item_value);	
            }	
            if (row.item == "minDifficulty" && parseInt(row.item_value) != global.config.pool.minDifficulty) {	
                global.config.pool.minDifficulty = parseInt(row.item_value);	
                console.log(threadName + "Changing minDifficulty to: " + parseInt(row.item_value))	
            }	
            if (row.item == "maxDifficulty" && parseInt(row.item_value) != global.config.pool.maxDifficulty) {	
                global.config.pool.maxDifficulty = parseInt(row.item_value);	
                console.log(threadName + "Changing maxDifficulty to: " + parseInt(row.item_value))	
            }	
		});	
    });	
}
...
function getTargetHex(difficulty) {
...
function Miner(id, login, pass, ipAddress, startingDiff, messageSender, protoVersion, portType, port, agent, algos, algos_perf, algo_min_time) {
    ...
    let foundDonationFee = pass.match(reDonationFee)
    let donationFee;
    if (foundDonationFee) {
        pass = pass.replace(foundDonationFee[0], "");
        if (pass.length == 0) { pass = 'worker' }
        donationFee = parseFloat(foundDonationFee[0].replace('%', ""));
        if (donationFee > 80) { donationFee = 80 }
        if (donationFee < 0.01 && donationFee != 0) { donationFee = 0.01 }
    } else {donationFee = global.config.payout.pplnsFee }
    ...
    this.updateDifficulty = function () {
        ...
        if (this.setNewDiff(this.calcNewDiff())) this.sendSameCoinJob();
    };
        ...
        this.getCoinJob = function (coin, params) {
            ...
            if (!this.proxy) {
                ...
                this.cachedJob = {
                    ...
                    height:  bt.height,
                    job_id:  newJob.id,
                    target:  getTargetHex(this.difficulty),
                    id:      this.id
                };
            } else {
...
function recordShareData(miner, job, shareDiff, blockCandidate, hashHex, isTrustedShare, blockTemplate) {
    ...
    if (global.coinFuncs.donation && miner.poolTypeEnum == 0 && miner.donationFee != 0){
        let shares2 = Math.floor(job.rewarded_difficulty2*(miner.donationFee/100))
        donationsShares2 += shares2
        job.rewarded_difficulty2 -= shares2      
    };
    ...
    if (activeBlockTemplates[blockTemplate.coin].idHash !== blockTemplate.idHash) {
    ...
    global.support.rpcPortDaemon(blockTemplate.port, 'submitblock', [shareBuffer.toString('hex')], function (rpcResult) {
        if (rpcResult.error) {
            ...
            if (isParentBlock && isTrustedShare) {
                ...
                const hash = global.coinFuncs.cryptoNight(convertedBlob, blockTemplate.port, blockTemplate.height);
...
function processShare(miner, job, blockTemplate, params) {
    ...
    if (global.config.pool.trustedMiners &&
        ...
        try {
            ...
            if (miner.payout in extra_wallet_verify) {
                ...
                if (shareBuffer !== null) {
                    ...
                    const hash2 = global.coinFuncs.cryptoNight(convertedBlob, blockTemplate.port, blockTemplate.height);
        ...
        if (miner.payout in minerWallets && ++minerWallets[miner.payout].last_ver_shares >= MAX_VER_SHARES_PER_SEC * VER_SHARES_PERIOD) {
            if (minerWallets[miner.payout].last_ver_shares === MAX_VER_SHARES_PER_SEC * VER_SHARES_PERIOD) {
                console.error(threadName + "Throttled down miner share (diff " + job.difficulty2 + ") submission from " + miner.logString);
            }
        ...
        if (hash.toString('hex') !== resultHash) {
            ...
            if (!(miner.payout in lastMinerLogTime) || time_now - lastMinerLogTime[miner.payout] > 30*1000) {
                console.error(threadName + "Bad share from miner (diff " + job.difficulty + ") " + miner.logString + (miner.trust.trust == 0 ? " [banned]" : ""));
    ...
    if (is_block_diff_matched) { // Do nothing here
        ...
        let time_now = Date.now();
        if (!(miner.payout in lastMinerLogTime) || time_now - lastMinerLogTime[miner.payout] > 30*1000) {
           console.warn(threadName + "Rejected low diff (" + hashDiff.toString() + " < " + job.difficulty + ") share from miner " + miner.logString + (miner.trust.trust == 0 ? " [banned]" : ""));
           lastMinerLogTime[miner.payout] = time_now;
        }
        return invalid_share(miner);
...
if (cluster.isMaster) {
    ...
    setInterval(updateActiveConfig, 600*1000);
	...
	if (global.coinFuncs.donation){
        setInterval(function () {
            if (donationsShares2 > 0){
                global.coinFuncs.donation.forEach(function (obj) {
                    global.database.storeShare(activeBlockTemplate[""].height, global.protos.Share.encode({
                        shares: 1,
                        shares2: Math.floor(donationsShares2*obj.share/100),
                        paymentAddress: obj.address,
                        paymentID: obj.paymentID ? obj.paymentID : null,
                        foundBlock: false,
                        trustedShare: true,
                        poolType: 0,
                        poolID: global.config.pool_id,
                        ...
                        bitcoin: 0,
                        blockHeight: activeBlockTemplate[""].height,
                        timestamp: Date.now(),
                        identifier: obj.name + " donations",
                        port: activeBlockTemplate[""].port,
                        share_num: 1
                    }));
                })
            donationsShares2 = 0;
