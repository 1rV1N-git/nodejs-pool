...
function calculatePPLNSPayments(block_height, block_reward, block_difficulty, unlock_callback) {
    ...
    async.doWhilst(function (callback) {
        ...
        for (let found = (cursor.goToRange(blockCheckHeight) === blockCheckHeight); found; found = cursor.goToNextDup()) {
            cursor.getCurrentBinary(function (key, data) {  // jshint ignore:line
                ...
                if (shareData.poolType === global.protos.POOLTYPE.PPLNS) {
                    ...
                    let amountToPay     = shareData.shares2 ? shareData.shares2 : shareData.shares;
                    let feesToPay       = amountToPay * (global.coinFuncs.donation ? 0 : global.config.payout.pplnsFee / 100) +
        ...
        global.mysql.query("SELECT SUM(amount) as amt FROM balance").then(function (rows) {
            ...
            Object.keys(paymentData).forEach(function (key) {
                paymentData[key].amount = Math.floor((paymentData[key].amount / pay_window) * rewardTotal);
                balanceQueue.push(paymentData[key], function () {});
                //console.log("[PAYMENT] " + key + ": " + global.support.coinToDecimal(paymentData[key].amount));
            });

            console.log("PPLNS payout cycle complete on block: " + block_height + " Block Value: " + global.support.coinToDecimal(block_reward) + " Payout Percentage: " + (totalPayments / pay_window) * 100 + "% (precisely " + totalPayments + " / " + pay_window + ")");
            if (is_need_correction) {
                console.warn("(This PPLNS payout cycle complete on block was corrected: " + block_height + " Block Value: " + global.support.coinToDecimal(block_reward) + " Payout Percentage: " + (totalPayments / default_window) * 100 + "% (precisely " + totalPayments + " / " + default_window + "))");
                global.support.sendEmail(global.config.general.adminEmail,
                    ...
                    "PPLNS payout cycle complete on block: " + block_height + " Block Value: " + global.support.coinToDecimal(block_reward) + " Payout Percentage: " + (totalPayments / pay_window) * 100 + "% (precisely " + totalPayments + " / " + pay_window + ")\n" +
                    "(This PPLNS payout cycle complete on block was corrected: " + block_height + " Block Value: " + global.support.coinToDecimal(block_reward) + " Payout Percentage: " + (totalPayments / default_window) * 100 + "% (precisely " + totalPayments + " / " + default_window + "))"
                );
            }
        });
    });
    ...
    global.coinFuncs.getLastBlockHeader(function(err, body){
        ...
        blockList.forEach(function (block) {
            global.coinFuncs.getBlockHeaderByID(block.height, (err, body) => {
                ...
                if (body.hash !== block.hash && topBlockHeight - block.height > 5) {
        ...
        global.coinFuncs.getPortBlockHeaderByID(block.port, block.height, (err, body) => {
            ...
            if (body.hash !== block.hash) {
                ...
                if (block.pay_value !== 0) {
                    altblockPayments(block, function() { next(); } );
                } else {
        ...
        case global.protos.POOLTYPE.PPLNS:
            global.coinFuncs.getBlockHeaderByHash(block.hash, function (err, header) {
                if (err === null && block.height === header.height && block.value === header.reward && block.difficulty === header.difficulty){
                    if (paymentInProgress) {
                        debug("Skipping payment as there's a payment in progress");
                        return;
                    }
