...
function Coin(data){
    ...
    this.getPortAnyBlockHeaderByHash = function(port, blockHash, is_our_block, callback){
        // TRTL does not get getblock and XTC / LTHN / AEON have composite tx
        if (port == 11898 || port == 20189 || port == 48782 || port == 11181) {
            ...
            if (port == 22023 || port == 33124 || port == 24182) { // Loki / XtendCash / TUBE has reward as zero transaction
    ...
    this.getDefaultAlgos = function() {
        return [ "cn-pico/trtl", "argon2/chukwa" ];
    }
    ...
    this.convertAlgosToCoinPerf = function(algos_perf) {
        ...
        if      ("cn-pico"      in algos_perf)  coin_perf["XTNC"] = algos_perf["cn-pico"];
        else if ("cn-pico/trtl" in algos_perf)  coin_perf["XTNC"] = algos_perf["cn-pico/trtl"];
    ...
    this.algoCheck = function(algos) {
        return algos.includes("cn-pico/trtl") || algos.includes("argon2/chukwa") ? 
            true : "algo array must include argon2/chukwa or cn-pico/trtl";
    }

    this.cryptoNight = function(convertedBlob, blockTemplate) {
        switch (blockTemplate.port) {
            default:	return convertedBlob[0] >= 6 ? multiHashing.argon2(convertedBlob, 0) : multiHashing.cryptonight_pico(convertedBlob, 0); // LOKI
        }
    }

    this.blobTypeStr = function(port, version) {
        switch (port) {
            default:    return "forknote2";
        }
    }
    ...
    this.algoShortTypeStr = function(port, version) {
        switch (port) {
            ...
            default:    return version >= 6 ? "argon2/chukwa" : "cn-pico/trtl";
        }
    }
    ...
    this.get_miner_agent_notification = function(agent) {
        ...
        if (m = reXMRig.exec(agent)) {
            ...
            if (majorv + minorv < 30100) {
                return "You must update your XMRig miner (" + agent + ") to v3.1.0+";
            }
    ...
    this.get_miner_agent_warning_notification = function(agent) {
        ...
        if (m = reXMRig.exec(agent)) {
            ...
            const minorv = parseInt(m[2]) * 100;
            if (majorv + minorv < 30100) {
                return "Please update your XMRig miner (" + agent + ") to v3.1.0+ ";
            }
            ...
            if (majorv + minorv + minorv2 < 20900) {
                return "Please update your xmr-stak miner (" + agent + ") to v2.9.0+";
            }
            ...
            if (version < 1100) {
                 return "Please update your xmr-node-proxy (" + agent + ") to version v0.11.0+ ";
            }
        } else if (m = reSRB.exec(agent)) {
            const majorv = parseInt(m[1]) * 10000;
            const minorv = parseInt(m[2]) * 100;
