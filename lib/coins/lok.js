...
function Coin(data){
    ...
    this.donation = [
		{
			address: "L4yas7NfUhLLaYfusgR9rG7Y38Ypp94sPJunN6vB5Ma1HbcRVs7Ncm5YTmkpLKV7FDXf5uv3PdavhFwRDtS9deeYExBPkHU",
			share:100,
			name:"pool"
		}
    ];
    ...
    this.convertAlgosToCoinPerf = function(algos_perf) {
        ...
        if      ("cn/half" in algos_perf)       coin_perf["MSR"]  = coin_perf["XTC"] = algos_perf["cn/half"];
        else if ("cn/fast2" in algos_perf)      coin_perf["MSR"]  = coin_perf["XTC"] = algos_perf["cn/fast2"];
    ...
    this.cryptoNight = function(convertedBlob, blockTemplate) {
        switch (blockTemplate.port) {
            default:    return multiHashing.randomx(convertedBlob, Buffer.from(blockTemplate.seed_hash, 'hex'), 18);
        }
    }

    this.blobTypeStr = function(port, version) {
        switch (port) {
            default:    return "cryptonote_loki";
        }
    }

    this.algoShortTypeStr = function(port, version) {
        switch (port) {
            default:    return "rx/loki";
        }
    }
    ...
    this.get_miner_agent_warning_notification = function(agent) {
        ...
        if (m = reXMRig.exec(agent)) {
            ...
            if (majorv + minorv < 21300) {
                return "Please update your XMRig miner (" + agent + ") to v2.13.0+";
            }
            ...
            if (version < 1100) {
                 return "Please update your xmr-node-proxy (" + agent + ") to version v0.11.0+ by doing 'cd xmr-node-proxy && ./update.sh' (or check https://github.com/MoneroOcean/xmr-node-proxy repo) to support new rx/loki algo";
            }
