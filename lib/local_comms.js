...
function Database(){
                ...
                cursor.getCurrentBinary(function (key, data) {  // jshint ignore:line
                    ...
                    if (blockData.poolType === pool_type || pool_type === false) {
                        response.push({
                            ...
                            paymentAddress: blockData.paymentAddress ? blockData.paymentAddress.substr(0,6) + '...' + blockData.paymentAddress.substr(-6): '',
                            paymentId: blockData.paymentId ? blockData.paymentId.substr(-6) : null,
                            worker: blockData.worker ? blockData.worker : ''
                        });
                    }
                });
            }
                ...
                for (let key in shares){
                    if (shares.hasOwnProperty(key)){
                        ...
                        shares[key].forEach(function(final_share){  // jshint ignore:line
                            ...
                            if (final_share.foundBlock) {
                                let msgPaymentID = "";
                                let poolType = '';
                                if (final_share.paymentID) {msgPaymentID = " paymentID: " +  final_share.paymentID.substr(-6)} else {msgPaymentID = ""}
                                switch (final_share.poolType) {
                                    case 0:
										poolType = "PPLNS ";
										break;
                                    case 1:
                                ...
                                let msgtext = poolType + "Block " + final_share.blockHeight + " was mined by " + final_share.paymentAddress.substr(-6) + msgPaymentID + " worker: " + final_share.identifier
                                let msgtextDiscord = poolType + "Block **" + global.config.general.coinCode + "** on height ``" + final_share.blockHeight + "`` was mined by " + final_share.paymentAddress.substr(-6) + msgPaymentID + " worker: " + final_share.identifier;
                                try {global.support.sendtoDiscord(msgtextDiscord);} catch (e) {console.error(e);}
                                try {global.support.sendtoTelegram(global.config.chat_id, msgtext);} catch (e) {console.log(e);}
                                setTimeout(function () { global.database.addWhoFound(final_share.blockHeight,final_share.paymentAddress,final_share.paymentID,final_share.identifier) }, 5*1000)
                            }
    ...
    this.addWhoFound = function (blockId, paymentAddress, paymentId, worker){
        this.refreshEnv();
        let txn = global.database.env.beginTxn();
        if (txn.getBinary(global.database.blockDB, blockId) == null) {
                    txn.abort();
                    console.error("Block " + blockId + " don't stored yet");
                    setTimeout(function () { global.database.addWhoFound(blockId, paymentAddress, paymentId, worker) }, 10*1000);
                    return;
        }
        let blockData = global.protos.Block.decode(txn.getBinary(global.database.blockDB, blockId));
        blockData.paymentAddress = paymentAddress;
        blockData.paymentId = paymentId ? paymentId : null;
        blockData.worker = worker ? worker : 'x';
        blockData = global.protos.Block.encode(blockData);
    ...
    this.storeAltBlock = function(blockId, blockData, callback){
        ...
        try{
            ...
            global.coinFuncs.getPortBlockHeaderByHash(blockDataDecoded.port, blockDataDecoded.hash, function(err, header){
                ...
                if (err || typeof(header) === 'undefined' || !header) { // bad blcok and not orphan
