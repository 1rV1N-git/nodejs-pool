        ...
        function (height, callback) {
			...
			const identifier = share.identifier;
                ...
                for (let miner in prevMinerSet) {
                    ...
                    get_telegram_id(address, function (telegram_id) {
                        console.log("telegram_id: " + telegram_id + " for miner: " + miner );
                        setTimeout(delayed_send_worker_stopped_hashing_telegram, 5*1000, miner, telegram_id, currentTime);
                    });
                }
                ...
                for (let miner in minerSet) {
                    ...
                    get_telegram_id(address, function (telegram_id) {
                        console.log("telegram_id: " + telegram_id + " for miner: " + miner );
                        send_worker_started_hashing_telegram(miner, telegram_id, currentTime);
                    });
                }
                ...
                if (!prev_pool_state_time || currentTime - prev_pool_state_time > hashrate_avg_min*60*1000) {
                    let pool_hashrate_ratio = prev_pool_hashrate > 5000 ? pool_hashrate / prev_pool_hashrate : 1;
                    let pool_workers_ratio  = prev_pool_workers > 20 ? pool_workers / prev_pool_workers : 1;
...
let minerEmail = {};
let minerTelegram_id = {};
// time of last SQL check for specific address
let minerEmailTime = {};
let minerTelegramTime = {};

// worker name -> time
let workers_started_hashing_time = {};
let workers_stopped_hashing_email_time = {};

let workers_started_hashing_time_tlg = {};
let workers_stopped_hashing_tlg_time = {};
...
function get_telegram_id(address, callback) {
    let currentTime = Date.now();
    if (!(address in minerTelegramTime) || currentTime - minerTelegramTime[address] > 10*60*1000) {
        minerTelegramTime[address] = currentTime;
        global.mysql.query("SELECT telegram_id FROM users WHERE username = ? AND enable_tlg IS true limit 1", [address]).then(function (rows) {
            if (rows.length === 0) {
                delete minerTelegram_id[address];
                return;
            } else {
                minerTelegram_id[address] = rows[0].telegram_id;
            }
            callback(minerTelegram_id[address]);
        });
    } else if (address in minerTelegram_id) {
        callback(minerTelegram_id[address]);
    }
}
...
function send_worker_started_hashing_telegram(miner, telegram_id, currentTime) {
    workers_started_hashing_time_tlg[miner] = currentTime;
    if (miner in workers_stopped_hashing_tlg_time) {
        delete workers_stopped_hashing_tlg_time[miner];
        ...
        global.support.sendtoTelegram(telegram_id, "Your worker: " + worker + " has started submitting hashes at: " + global.support.formatDate(currentTime) + " UTC");
    }
}
...
function delayed_send_worker_stopped_hashing_telegram(miner, telegram_id, currentTime) {
    if (miner in workers_started_hashing_time_tlg && Date.now() - workers_started_hashing_time_tlg[miner] <= 5*60*1000) {
        delete workers_started_hashing_time_tlg[miner];
        return;
    }

    delete workers_started_hashing_time_tlg[miner];
    workers_stopped_hashing_tlg_time[miner] = Date.now();
	...
	global.support.sendtoTelegram(telegram_id, "Your worker: " + worker + " has stopped submitting hashes at: " + global.support.formatDate(currentTime) + " UTC"); 
}
...
function updatePoolStats2(poolType) {
    ...
    async.series([
        ...
        function (callback) {
            let portCoinAlgo = {};
            for (let port in global.coinFuncs.getPORTS()) portCoinAlgo[port] = global.coinFuncs.algoShortTypeStr(port, 0);
        ...
        global.coinFuncs.getPortLastBlockHeader(port, function(err, body){
            global.support.rpcPortDaemon(port,'get_info', [], function (rpcResult) {
                ...
                if (-- left === 0) {
                    ...
                    async.series([
                    function (callback) {
                        global.mysql.query("SELECT (SUM(difficulty)/COUNT(id)) as avgdiff FROM block_log WHERE find_time >= NOW() - INTERVAL 7 DAY").then(function(row) {
                            avgDiff.pastWeek = parseInt(row[0].avgdiff);
                        }).then(function () {
                            return callback(null);
                        });
                    },
                    function (callback) {
                        global.mysql.query("SELECT (SUM(difficulty)/COUNT(id)) as avgdiff FROM block_log WHERE find_time >= NOW() - INTERVAL 1 DAY").then(function(row) {
                            avgDiff.pastDay = parseInt(row[0].avgdiff);
                        }).then(function () {
                            return callback(null);
                        });
                    }
                    ], function () {
                        info.avgDiffPastWeek          = avgDiff.pastWeek;
                        info.avgDiffPastDay           = avgDiff.pastDay;
